        ...
        public IAxon Create(IList<Synapse> terminals, Type activationFunction)
        {
            ...
            if (!(functionObj is IActivationFunction))
            {
                throw new NotSupportedException(
                    string.Format("{activationFunction} is not a supported activation function type for Create() as it does not implement IActivationFunction", activationFunction));
            }
